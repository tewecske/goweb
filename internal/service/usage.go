...
// string, credential, or bearer value.
...
func NewUsageQueue(repository UsageEventRepository, capacity int) (*UsageQueue, error) {
	...
	return &UsageQueue{repository: repository, events: make(chan UsageEvent, capacity), now: time.Now}, nil
}
...
func (q *UsageQueue) persist(ctx context.Context, event UsageEvent) {
	if err := q.repository.RecordUsageEvent(ctx, event); err != nil {
		q.failed.Add(1)
		if q.logger != nil {
			q.logger.Error("usage event storage failed", "error", err)
		}
		return
	}
	q.recorded.Add(1)
