...
- Production startup fails through `config.ErrUnsafeProductionConfig` unless `GOWEB_SESSION_COOKIE_SECURE` is true, `GOWEB_PUBLIC_URL` uses HTTPS, `GOWEB_DATABASE_URL` and `GOWEB_SESSION_SECRET` are set, and `GOWEB_MAIL_RELAY` is set whenever email confirmation is required.
...
- Browser runs require external mail/provider/database fixtures from environment when exercising persistence-backed flows; never commit credentials, tokens, or bearer fixtures.
...
Read `docs/architecture.md`, `docs/dependency-policy.md`, `docs/local-development.md`, `docs/deployment.md`, `docs/bootstrap-admin.md`, `docs/backup-retention.md`, and `docs/threat-model.md` before changing related boundaries. Update documentation when commands, configuration, package responsibilities, deployment steps, or security controls change.
